...
static TokenType checkKeyword(int start, int length, const char* rest, TokenType type) {
  // First, ensure we have the right number of characters
  // Second, make sure they are the correct characters
...
static TokenType identifierType() {
  switch (scanner.start[0]) {
    ...
    case 'f': {
      // safety first on that indexing - 'f' is a valid identifier
    ...
    case 't': {
      // safety first on that indexing - 't' is a valid identifier
      if (scanner.current - scanner.start > 1) {
        switch (scanner.start[1]) {
